...
public class JsonStreamSerializerUnitTest {

    JsonStreamSerializer serializer;
    ...
    public void testCircularSerialize() throws Exception {
        ...
        try {
            ...
            TestSerializerSample innerTwo = new TestSerializerSample();
            innerTwo.bigint1 = 100L;
            innerTwo.boolean1 = true;
            innerSample.innerSample = innerTwo;

            expected.innerSample = innerSample;

            Record2Container container = new Record2Container();
            List<Record2> record2s = new ArrayList<>();

            // generate a record
            Record2 record2 = new Record2();
            record2.bigint2 = 1024 * 1024 * 32L;
            List<byte[]> bytes = new ArrayList<byte[]>();
            bytes.add("testBytes1".getBytes());
            bytes.add("testBytes2".getBytes());
            bytes.add("testBytes3".getBytes());
            record2.byteslist = bytes;
            record2.enum2 = Enum2Values.PLANE;
            record2.list2 = Arrays.asList(1, 3, 5);
            Record record = new Record(1, true, "testRecord");
            Map<String, Record> mapRecord = new HashMap<String, Record>();
            mapRecord.put("1", record);
            mapRecord.put("2", record);
            record2.put("map2", mapRecord);
            record2.nullablebigint = 1024 * 1024 * 32L;

            record2s.add(record2);
            record2s.add(record2);
            container.record2list = record2s;

            expected.container1 = container;
            ...
            Assert.assertNotNull(actual.innerSample.innerSample);
            Assert.assertEquals(expected.innerSample.innerSample.bigint1, actual.innerSample.innerSample.bigint1);
            Assert.assertEquals(expected.innerSample.innerSample.boolean1, actual.innerSample.innerSample.boolean1);
            Assert.assertNotNull(actual.container1);
            for (int i = 0; i < expected.container1.record2list.size(); i++) {
                verifyRecord2(expected.container1.record2list.get(i), actual.container1.record2list.get(i));
            }
        } finally {
            if (os != null) {
                os.close();
    ...
    private void verifyRecord2(Record2 expected, Record2 actual) {
        Assert.assertEquals(expected.bigint2, actual.bigint2);
        Assert.assertEquals(expected.enum2, actual.enum2);
        Assert.assertEquals(expected.nullablebigint, actual.nullablebigint);
        for (int i = 0; i < expected.byteslist.size(); i++) {
